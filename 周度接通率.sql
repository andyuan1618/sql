...
,isnull(sum(case when c.calldate is not null then 1 else 0 end),0) '拨打话单量'
,isnull(sum(c.isgetthrough),0) '接通客户数'
,case when isnull(sum(case when c.calldate is not null then 1 else 0 end),0)=0 then 0 else isnull(sum(c.isgetthrough),0) *1./isnull(sum(case when c.calldate is not null then 1 else 0 end),0) end '接通率'
